...
p_hist <- ggplot(data.frame(res = res_vec),aes(x = res)) +
  ...
  labs(title = "Sudeste", x = "Resíduo",y = "Densidade")+
  theme(
...
p_qq <- ggplot(data.frame(res = res_vec), aes(sample = res)) +
  ...
  labs( title = "Sudeste",  x = "Quantis teóricos",y = "Quantis observados" )+
  theme(
    panel.grid.minor = element_blank(),
    plot.title = element_text(size = 12),
    axis.title = element_text(size = 12),
...
p_res_fit <- ggplot(df_res_fit, aes(x = log_mx, y = res)) +
            ...
            theme_minimal() +
  scale_y_continuous(breaks = seq(-3, 3, by = 1), limits = c(-3, 3.4))+
  labs(title = "Sudeste", x = "ln(m̂ₓ,ₜ)",  y = "Resíduo")+
  theme(
    panel.grid.minor = element_blank(),
    plot.title = element_text(size = 12),
    axis.title = element_text(size = 12),
...
shapiro_test_sd  <- shapiro_test
shapiro_test_sd 

# AUTOCORRELAÇÃO DOS RESÍDUOS — DIAGNÓSTICO (ACF)

p_acf_sd <- ggAcf(res_vec, lag.max = 20) + 
  scale_y_continuous(limits = c(-0.5, 0.5)) +
  labs(title = "Sudeste", x = "Defasagem", y = "ACF")+
  theme_bw() +
  theme(
...
p_res_age <- ggplot(res_df, aes(x = factor(IDADE), y = RES)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme_minimal() +
  labs(title = "Resíduos por idade - Sudeste", x = "Idade central", y = "Resíduo") +
