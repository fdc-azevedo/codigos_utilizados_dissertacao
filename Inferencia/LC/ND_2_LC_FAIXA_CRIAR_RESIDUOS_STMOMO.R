...
p_hist <- ggplot(data.frame(res = res_vec),aes(x = res)) +
  ...
  scale_y_continuous(breaks = seq(0, 0.6, by = 0.2), 
                     limits = c(0, 0.65))+
  labs(title = "Nordeste", x = "Resíduo",y = "Densidade")+
  theme(
...
p_qq <- ggplot(data.frame(res = res_vec), aes(sample = res)) +
  ...
  labs( title = "Nordeste",  x = "Quantis teóricos",y = "Quantis observados" )+
  theme(
    panel.grid.minor = element_blank(),
    plot.title = element_text(size = 12),
    axis.title = element_text(size = 12),
...
p_res_fit <- ggplot(df_res_fit, aes(x = log_mx, y = res)) +
            ...
            theme_minimal() +
  scale_y_continuous(breaks = seq(-3, 3, by = 1), limits = c(-3, 3.4))+
  labs(title = "Nordeste", x = "ln(m̂ₓ,ₜ)",  y = "Resíduo")+
  theme(
    panel.grid.minor = element_blank(),
    plot.title = element_text(size = 12),
    axis.title = element_text(size = 12),
...
shapiro_test_nd <- shapiro_test
shapiro_test_nd

# AUTOCORRELAÇÃO DOS RESÍDUOS — DIAGNÓSTICO (ACF)

p_acf_nd <- ggAcf(res_vec, lag.max = 20) + 
  scale_y_continuous(limits = c(-0.5, 0.5)) +
  labs(title = "Nordeste", x = "Defasagem", y = "ACF")+
  theme_bw() +
  theme(
...
p_res_age <- ggplot(res_df, aes(x = factor(IDADE), y = RES)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme_minimal() +
  labs(title = "Resíduos por idade - Nordeste", x = "Idade central", y = "Resíduo") +
  theme(axis.text.x = element_text(angle = 0, hjust = 1))

#p_res_age
#p_res_age_nd <- p_res_age