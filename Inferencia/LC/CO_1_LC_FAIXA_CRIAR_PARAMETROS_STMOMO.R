...
p_taxa_idade_centro <- p_taxa_idade
...
p_ax <- ggplot(df_ax, aes(x = IDADE, y = ax)) +
  geom_line(linewidth = 1.0) +
  geom_point(size = 2) +
  labs(title = "Centro-Oeste", x = "Idade central", y = expression(alpha[x])) +
  theme_minimal() +
  theme(
...
p_bx <- ggplot(df_bx, aes(x = IDADE, y = bx)) +
  ...
  labs(title = "Centro-Oeste", x = "Idade central", y = expression(b[x])) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, vjust = 1, size = 12),
    axis.text.y = element_text(size = 12),
