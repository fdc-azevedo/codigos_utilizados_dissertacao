...
# Pesos etários fixos em 2024 (Sudeste)
w_idade <- dados_prep_obs %>%
  filter(LOCAL == "Sudeste", ANO == ano_base_pesos) %>%
...
taxa_historica <- dados_prep_obs %>%
  filter(LOCAL == "Sudeste", ANO %in% anos_obs) %>%
...
p_forecast_sd <- ggplot() +
  ...
  labs(title = "Sudeste", x = "Ano", y = "Taxa por 100.000 mulheres", color = "Período") +
  theme_minimal() +
...
# RMSE
RMSE_sd <- sqrt(mean((y_hat - y_obs)^2))

# MAPE
MAPE_sd <- mean(abs((y_obs - y_hat) / y_obs)) * 100
