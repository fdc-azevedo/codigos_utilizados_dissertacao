...
# Pesos etários fixos em 2024 (Nordeste)
w_idade <- dados_prep_obs %>%
  filter(LOCAL == "Nordeste", ANO == ano_base_pesos) %>%
...
taxa_historica <- dados_prep_obs %>%
  filter(LOCAL == "Nordeste", ANO %in% anos_obs) %>%
...
p_forecast_nd <- ggplot() +
  ...
  labs(title = "Nordeste", x = "Ano", y = "Taxa por 100.000 mulheres", color = "Período") +
  theme_minimal() +
...
# RMSE
RMSE_nd <- sqrt(mean((y_hat - y_obs)^2))

# MAPE
MAPE_nd <- mean(abs((y_obs - y_hat) / y_obs)) * 100
