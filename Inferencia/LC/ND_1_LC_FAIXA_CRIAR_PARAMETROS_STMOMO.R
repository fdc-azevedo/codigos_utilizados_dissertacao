...
# 1) PREPARAR DADOS (mantém tua lógica)
...
dados_nordeste <- dados_prep %>%
  filter(LOCAL == "Nordeste")

stopifnot(all(is.finite(dados_nordeste$IDADE_CENTRAL)))
stopifnot(all(is.finite(dados_nordeste$ANO)))
...
cat("Resultados do fit:\n")
cat("- conv:", LCfit$conv, "\n")
cat("- fail:", LCfit$fail, "\n")
cat("- deviance:", round(LCfit$deviance, 2), "\n")
...
p_ax <- ggplot(df_ax, aes(x = IDADE, y = ax)) +
  geom_line(linewidth = 1.0) +
  geom_point(size = 2) +
  labs(title = "Nordeste", x = "Idade central", y = expression(alpha[x])) +
  theme_minimal() +
  theme(
...
p_bx <- ggplot(df_bx, aes(x = IDADE, y = bx)) +
  ...
  labs(title = "Nordeste", x = "Idade central", y = expression(b[x])) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, vjust = 1, size = 12),
    axis.text.y = element_text(size = 12),
