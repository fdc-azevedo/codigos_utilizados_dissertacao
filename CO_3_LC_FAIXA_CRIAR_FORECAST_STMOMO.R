...
# - Série observada 2000–2024 (bruta) padronizada por idade
# - Pesos fixos em 2024 (ano-base)
...
# -----------------------------------------------------------------------------
# Obs: dados_prep do Step 1 é 2000–2019. Aqui você precisa do 2000–2024.
...
# Pesos etários fixos em 2024 (Centro-Oeste)
w_idade <- dados_prep_obs %>%
  filter(LOCAL == "Centro-Oeste", ANO == ano_base_pesos) %>%
...
taxa_historica <- dados_prep_obs %>%
  filter(LOCAL == "Centro-Oeste", ANO %in% anos_obs) %>%
...
p_forecast_centro <- ggplot() +
  ...
  scale_y_continuous(limits = c(7, 57), breaks = seq(10, 50, by = 10)) +
  labs(title = "Centro-Oeste", x = "Ano", y = "Taxa por 100.000 mulheres", color = "Período") +
  theme_minimal() +
...
# RMSE
RMSE_centro <- sqrt(mean((y_hat - y_obs)^2))

# MAPE
MAPE_centro <- mean(abs((y_obs - y_hat) / y_obs)) * 100