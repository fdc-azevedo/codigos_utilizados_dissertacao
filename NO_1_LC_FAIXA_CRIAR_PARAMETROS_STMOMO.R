...
dados_norte <- dados_prep %>%
  filter(LOCAL == "Norte")

stopifnot(all(is.finite(dados_norte$IDADE_CENTRAL)))
stopifnot(all(is.finite(dados_norte$ANO)))

cat("Dados após preparação:\n")
cat("- Período:", min(dados_norte$ANO), "-", max(dados_norte$ANO), "\n")
cat("- Faixas:", paste(sort(unique(dados_norte$FAIXA_ORIG)), collapse = ", "), "\n")
cat("- N linhas (Norte):", nrow(dados_norte), "\n\n")
...
cat("✅ StMoMoData pronto:", length(dados_stmomo$ages), "idades x", length(dados_stmomo$years), "anos\n")
...
cat("Período treino:", min(periodo_treino), "-", max(periodo_treino), "\n")
cat("Idades (fit):", min(idades_fit), "-", max(idades_fit), "\n")

# 5) AJUSTE LEE-CARTER (somente estimação de parâmetros)
set.seed(88)
LCfit <- fit(
  lc(link = "log"),
  data = dados_stmomo,
...
# -----------------------------------------------------------------------------
# 6) PARÂMETROS (ax, bx, kt) + gráficos (somente leitura/inspeção)
# -----------------------------------------------------------------------------

ax <- as.numeric(LCfit$ax)
bx <- as.numeric(LCfit$bx)         # lc() -> 1 coluna
kt <- as.numeric(LCfit$kt[1, ])    # lc() -> 1 linha
...
p_ax <- ggplot(df_ax, aes(x = IDADE, y = ax)) +
  geom_line(linewidth = 1.0) +
  geom_point(size = 2) +
  labs(title = "Norte", x = "Idade central", y = expression(alpha[x])) +
  theme_minimal() +
  theme(
...
p_bx <- ggplot(df_bx, aes(x = IDADE, y = bx)) +
  ...
  labs(title = "Norte", x = "Idade central", y = expression(b[x])) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, vjust = 1, size = 12),
    axis.text.y = element_text(size = 12),
