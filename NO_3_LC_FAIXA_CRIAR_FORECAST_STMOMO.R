...
# Pesos etários fixos em 2024 (Norte)
w_idade <- dados_prep_obs %>%
  filter(LOCAL == "Norte", ANO == ano_base_pesos) %>%
...
taxa_historica <- dados_prep_obs %>%
  filter(LOCAL == "Norte", ANO %in% anos_obs) %>%
...
p_forecast_no <- ggplot() +
  ...
  labs(title = "Norte", x = "Ano", y = "Taxa por 100.000 mulheres", color = "Período") +
  theme_minimal() +
...
# RMSE
RMSE_no <- sqrt(mean((y_hat - y_obs)^2))

# MAPE
MAPE_no <- mean(abs((y_obs - y_hat) / y_obs)) * 100
