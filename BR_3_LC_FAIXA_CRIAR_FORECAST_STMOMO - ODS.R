...
# Pesos etários fixos em 2024 (Brasil)
w_idade <- dados_prep_obs %>%
  filter(LOCAL == "Brasil", ANO == ano_base_pesos) %>%
...
taxa_historica <- dados_prep_obs %>%
  filter(LOCAL == "Brasil", ANO %in% anos_obs) %>%
...
# 5) META ODS 3.4 (redução de 1/3 vs 2014)
...
p_forecast_brasil <- ggplot() +
  ...
  geom_hline(yintercept = meta_ods_34,linetype = "dashed", linewidth = 0.9) +
  annotate("text",x = 2024, y = meta_ods_34,
           label = "ODS 3.4 = 18,84",  hjust = -0.05, vjust = -0.6, size = 3.6) +
  ...
  labs( x = "Ano", y = "Taxa por 100.000 mulheres", color = "Período") +
  theme_minimal() +
...
# RMSE
RMSE_brasil <- sqrt(mean((y_hat - y_obs)^2))

# MAPE
MAPE_brasil <- mean(abs((y_obs - y_hat) / y_obs)) * 100
